...
const Step7_GrupoSoat = () => {
  ...
  const runtVin = useSelector(selectVin);
  const runtNombres = useSelector(selectNombres);
  const runtApellidos = useSelector(selectApellidos);

  // Datos para el modal de validación (opciones 1 y 2 de Cero KM)
  ...
  // El modal de validación se muestra una sola vez por cotización resuelta.
  const [validacionMostrada, setValidacionMostrada] = useState(false);
  ...
  // ── Modal de validación (solo opciones 1 y 2 de Cero KM) ──
  // Se muestra al final, cuando la tarifa/precio ya están resueltos, para que el
  // operario valide placa, VIN, titular y precio antes de continuar.
  useEffect(() => {
    const esOpcion1o2 =
      tipoVehiculo === 'CERO_KM' &&
      (metodoConsulta === 'IA_VIN_RUNT' || metodoConsulta === 'PLACA_FALABELLA');

    if (!esOpcion1o2) return;
    if (validacionMostrada) return;
    if (requiereRevision) return;
    if (!tarifaCodigo || !tarifaDetalle) return; // esperar a que el precio esté resuelto

    setValidacionMostrada(true);

    const fmt = (v) =>
      new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP', maximumFractionDigits: 0 }).format(v || 0);

    const precio = totalCotizacion != null ? totalCotizacion : tarifaDetalle.valor;
    const placa = runtPlaca || '—';
    const vin = runtVin || '—';
    const last4 = (runtVin || '').slice(-4);
    const nombreTitular = `${runtNombres || ''} ${runtApellidos || ''}`.trim() || '(sin nombre)';
    const docLinea = consultaDocumento ? `${tipoDocumento || ''} ${consultaDocumento}`.trim() : '';
    const tel = consultaTelefono || celularCotizacion || '';

    const html = `
      <div style="text-align:center;">
        <div style="display:flex; gap:28px; justify-content:center; flex-wrap:wrap; margin-bottom:4px;">
          <div>
            <div style="font-size:0.8rem;color:#888;text-transform:uppercase;letter-spacing:0.5px;">Placa</div>
            <div style="font-size:2rem;font-weight:800;letter-spacing:1px;color:#212121;">${placa}</div>
          </div>
          <div>
            <div style="font-size:0.8rem;color:#888;text-transform:uppercase;letter-spacing:0.5px;">VIN</div>
            <div style="font-size:1.4rem;font-weight:700;color:#212121;">${vin}</div>
          </div>
        </div>

        <div style="margin:14px 0;">
          <div style="font-size:0.8rem;color:#888;text-transform:uppercase;letter-spacing:0.5px;">Titular</div>
          <div style="font-size:1.5rem;font-weight:700;color:#212121;">${nombreTitular}</div>
          ${docLinea ? `<div style="font-size:1.15rem;color:#555;">${docLinea}</div>` : ''}
          ${tel ? `<div style="font-size:1.15rem;color:#555;">TEL ${tel}</div>` : ''}
        </div>

        <div style="background:#fff8e1;border-left:4px solid #ff9800;border-radius:8px;padding:12px 14px;margin:16px 0;text-align:left;">
          <strong style="color:#e65100;font-size:1rem;">⚠ Valide en su factura/PDF o imagen que la placa coincida con el VIN **${last4}</strong>
        </div>

        <div style="margin-top:10px;">
          <div style="font-size:0.9rem;color:#888;text-transform:uppercase;letter-spacing:0.5px;">Precio</div>
          <div style="font-size:2.4rem;font-weight:800;color:#1976d2;">${fmt(precio)}</div>
        </div>
      </div>
    `;

    AlertService.confirm('Confirma los datos de la cotización', html, {
      icon: 'info',
    ...
    tipoVehiculo, metodoConsulta, validacionMostrada, requiereRevision,
    tarifaCodigo, tarifaDetalle, totalCotizacion,
    runtPlaca, runtVin, runtNombres, runtApellidos,
    tipoDocumento, consultaDocumento, consultaTelefono, celularCotizacion,
  ]);
