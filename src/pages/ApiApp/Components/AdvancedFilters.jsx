  ...
  const filterConfig = useMemo(() => [
    {
      ...
      type: 'text',
      label: 'Buscar',
      placeholder: 'Nombre, codigo o URL...',
      showSearchIcon: true,
      width: 4,
    },
    {
            ...
            <Grid container spacing={1}>
              <Grid item xs={6}>
                <DatePicker
                  ...
                  onChange={(v) => handleFilterChange(config.startField, v?.format('YYYY-MM-DD') || '')}
              ...
              <Grid item xs={6}>
                <DatePicker
                  ...
                  onChange={(v) => handleFilterChange(config.endField, v?.format('YYYY-MM-DD') || '')}
                  slotProps={{ textField: { size: 'small', fullWidth: true } }}
                />
              </Grid>
            </Grid>
      ...
      <Accordion expanded={isExpanded} onChange={() => setIsExpanded(!isExpanded)}>
        <AccordionSummary expandIcon={<ExpandMoreIcon />}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            {activeFilters.length > 0 && <Chip size="small" label={activeFilters.length} color="primary" />}
          </Box>
        </AccordionSummary>
