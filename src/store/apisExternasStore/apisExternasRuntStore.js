...
export const apisExternasRuntStore = createSlice({
  ...
  reducers: {
    ...
    setPersona: (state, action) => {
      const data = action.payload || {};
      state.id_persona = data.id_persona ?? null;
      state.tipo_documento_persona = data.tipo_documento ?? null;
      state.numero_documento_persona = data.numero_documento ?? null;
      state.nombres = data.nombres ?? null;
      state.apellidos = data.apellidos ?? null;
      state.fecha_inscripcion = data.fecha_inscripcion ?? null;
      state.estado_conductor = data.estado_conductor ?? null;
      state.estado_persona = data.estado_persona ?? null;
      state.mostrar_solicitudes = data.mostrar_solicitudes ?? null;
      state.no_inscrito = data.no_inscrito ?? null;
      state.tiene_licencias = data.tiene_licencias ?? null;
      state.tiene_multas = data.tiene_multas ?? null;
      state.nro_paz_y_salvo = data.nro_paz_y_salvo ?? null;
    },
